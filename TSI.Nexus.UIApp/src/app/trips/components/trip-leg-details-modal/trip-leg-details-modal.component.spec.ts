...
import { TripLegDetailsModalComponent } from './trip-leg-details-modal.component';

describe('TripLegDetailsModalComponent', () => {
  let dialogRefMock: { close: ReturnType<typeof vi.fn> };
  let tripLegServiceMock: { add: ReturnType<typeof vi.fn>; update: ReturnType<typeof vi.fn> };
  let notificationServiceMock: { showMessage: ReturnType<typeof vi.fn> };
  let translationServiceMock: { instant: ReturnType<typeof vi.fn> };
  let cdrMock: { markForCheck: ReturnType<typeof vi.fn> };

  function createComponent(dialogData: any): TripLegDetailsModalComponent {
    dialogRefMock = { close: vi.fn() };
    tripLegServiceMock = { add: vi.fn(), update: vi.fn() };
    notificationServiceMock = { showMessage: vi.fn() };
    translationServiceMock = { instant: vi.fn((key: string) => key) };
    cdrMock = { markForCheck: vi.fn() };

    return new TripLegDetailsModalComponent(
      dialogRefMock as unknown as MatDialogRef<TripLegDetailsModalComponent>,
      dialogData,
      new FormBuilder(),
  ...
  describe('submitAdd (private, via submit)', () => {
    function setupValidAddForm(nextSequenceNumber = 1) {
      const component = createComponent({ tripId: 't1', nextSequenceNumber });
      component.form.patchValue({ origin: 'A' });

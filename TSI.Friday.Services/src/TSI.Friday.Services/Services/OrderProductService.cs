    ...
    public class OrderProductService : IOrderProductService
    {
        ...
        public async Task<WebApiResponse<OrderProductDto>> Add(OrderProductDto orderProductDto)
        {
            ...
            try
            {
                ...
                result.Message = $"Item do Pedido {orderProductDto.Description} cadastrado com sucesso.";
            }
            catch (Exception ex)
            {
                result.Status = ResponseStatus.Error;
                result.Message = $"Não foi possível cadastrar o Item do Pedido {orderProductDto?.Description} na base de dados. Erro: {ex.Message}";
            }
        ...
        public async Task<WebApiResponse<OrderProductDto>> Update(OrderProductDto orderProductDto)
        {
            ...
            {
                var orderProductEntity = _mapper.Map<OrderProduct>(orderProductDto);
                await _repository.UpdateAsync(orderProductEntity);

                result.Data = orderProductDto;
                result.Status = ResponseStatus.Success;
                result.Message = $"Item do Pedido {orderProductDto.Description} atualizado com sucesso.";
            }
            catch (Exception ex)
            {
                result.Status = ResponseStatus.Error;
                result.Message = $"Não foi possível atualizar os dados do Item do Pedido {orderProductDto?.Description} na base de dados. Erro: {ex.Message}";
            }
        ...
        public async Task<WebApiResponse<OrderProductDto>> Remove(OrderProductDto orderProductDto)
        {
            ...
            try
            {
                ...
                result.Data = orderProductDto;
                result.Status = ResponseStatus.Success;
                result.Message = $"Item do Pedido {orderProductDto.Description} removido com sucesso.";
            }
            catch (Exception ex)
            {
                result.Status = ResponseStatus.Error;
                result.Message = $"Não foi possível remover o Item do Pedido {orderProductDto?.Description} da base de dados. Erro: {ex.Message}";
            }
        ...
        public async Task<WebApiResponse<IEnumerable<OrderProductDto>>> FindByOrderId(int? orderId)
        {
            ...
            {
                result.Status = ResponseStatus.Error;
                result.Message = $"Não foi possível acessar os Itens do Pedido {orderId}. Erro: {ex.Message}";
            }
        ...
        public async Task<WebApiResponse<OrderProductDto>> FindById(int? id)
        {
            ...
            {
                result.Status = ResponseStatus.Error;
                result.Message = $"Não foi possível acessar os registros de Itens do Pedido na base de dados. Erro: {ex.Message}";
            }
